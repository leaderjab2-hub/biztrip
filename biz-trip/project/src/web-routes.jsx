        ...
        return (
          <div key={`${route.fromSched}-${route.toSched}`} className="route-row">
            ...
            <div style={{ width: 176, display: "flex", gap: 8, flexShrink: 0 }}>
              <button className="adot-btn line" style={{ height: 32, padding: "0 10px", fontSize: 12, flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 5 }} onClick={() => window.openMapUrl("", `${from?.name || ""} ${to?.name || ""}`)}>
        ...
        <div className="content">
          <div style={{ display: "flex", alignItems: "flex-end", gap: 16 }}>
            <div>
              ...
              <div style={{ font: "500 12px/16px var(--font-pretendard)", color: "var(--on-surface-neutral-40)", marginTop: 8 }}>
                이동시간은 현재 수동 입력 기준으로 관리됩니다.
              </div>
            </div>
            <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
              <button className="adot-btn line" style={{ height: 34, padding: "0 12px", fontSize: 13, display: "inline-flex", alignItems: "center", gap: 6 }} onClick={() => {
                const labels = routes.slice(0, 2).flatMap(r => [window.TD.getPlace(r.from)?.name, window.TD.getPlace(r.to)?.name]).filter(Boolean).join(" ");
                window.openMapUrl("", labels);
              }}>
